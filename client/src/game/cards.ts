import { Card, PropertyColor, PropertySetInfo } from './types';
...
export function createDeck(): Card[] {
  ...
  for (let i = 0; i < 3; i++) {
    deck.push(actionCard('Pass Go', 1, 'pass_go', 'Draw 2 extra cards'));
  }
  for (let i = 0; i < 3; i++) {
    deck.push(actionCard('Debt Collector', 3, 'debt_collector', 'Force one player to pay you $5M'));
  }
  for (let i = 0; i < 2; i++) {
    deck.push(actionCard('Birthday!', 2, 'birthday', 'All players pay you $2M'));
  }
  for (let i = 0; i < 3; i++) {
    deck.push(actionCard('Sly Deal', 3, 'sly_deal', 'Steal a property from another player'));
  }
  for (let i = 0; i < 3; i++) {
    deck.push(actionCard('Forced Deal', 3, 'forced_deal', 'Swap a property with another player'));
  }
  for (let i = 0; i < 2; i++) {
    deck.push(actionCard('Deal Breaker', 5, 'deal_breaker', 'Steal a complete property set'));
  }
  for (let i = 0; i < 3; i++) {
    deck.push(actionCard('Just Say No', 4, 'just_say_no', 'Cancel any action against you'));
  }
  for (let i = 0; i < 3; i++) {
    deck.push(actionCard('House', 3, 'house', 'Add to a complete set: +$3M rent'));
  }
  for (let i = 0; i < 2; i++) {
    deck.push(actionCard('Hotel', 4, 'hotel', 'Add to a set with house: +$4M rent'));
  }
  ...
  deck.push(actionCard('Rent: Railroad/Util', 1, 'rent', 'Charge rent on Railroad or Utility', ['black', 'purple']));
  for (let i = 0; i < 3; i++) {
    deck.push(actionCard('Wild Rent', 3, 'wild_rent', 'Charge rent on any color you own'));
  }
  for (let i = 0; i < 2; i++) {
    deck.push(actionCard('Double Rent', 1, 'double_rent', 'Double the rent you charge'));
  }
